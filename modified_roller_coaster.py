...
def validate_csv(df):
    for i, row in df.iterrows():
        ...
        # Evaluate and compare end_x and start_x as floating point numbers
        start_x = sp.sympify(row['start_x']).evalf()
        end_x = sp.sympify(row['end_x']).evalf()
        if end_x <= start_x:
...
def main():
    ...
    file_name = 'roller_coaster_formulas.csv'
    df = pd.read_csv(file_name)
    validate_csv(df)
